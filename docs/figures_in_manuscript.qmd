...
crop_cols <- c(
    "#BF812D", "#543005",  "#a1d99b", "#31a354", "#fdae6b", 
...
vals <- vals %>% group_by(Element) %>% 
    ...
    mutate(Element = factor(Element, levels = c("Y", "B", "Ca", "Co", "D"),
                            labels = c("New cropland", "Biodiveristy loss", "Carbon loss",
                                       "Connectivity loss", "Cost distance increase")))

g2 <- ggplot(data = vals) + 
    geom_linerange(aes(
        x = Practice, ymin = 0, ymax = value, linetype = USG), 
        linewidth = 0.6, position = position_dodge2(width = 0.5)) +
    geom_point(aes(x = Practice, y = value), size = 1,
