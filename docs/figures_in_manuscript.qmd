...
fig_list <- lapply(c("Y", "I", "B", "Ca", "Co", "D"), function(item){
    ...
    vals <- vals %>% 
        ...
        mutate(group = factor(
            ...
            labels = c("Single factor (", "Optimal)", "Combined factors")))
...
## Connectivity contribution - results
vals_hh <- vals_cmpr %>% filter(scenario %in% c("YBCaCoD", "YBCaD"))

(vals_hh[1, -6] - vals_hh[2, -6]) / vals_hh[2, -6]
```
...
fig_list <- lapply(names(simus)[1:5], function(ele){
    ...
    p <- ggplot(vals, aes(x = farmable_area, y = value)) + 
        ...
        scale_color_npg(name = "Solution") +
        scale_linetype_manual(
            name = "Solution", values = c(rep("solid", 5), rep("dashed", 4)))
...
g1 <- ggplot() +
    ...
    geom_point(data = vals %>% filter(type == "point"), 
              aes(x = weight, y = farmable_area, color = group)) +
    xlab("Weight") + ylab("Expand area increase (%)") +
...
g2 <- ggplot() +
    ...
    geom_point(data = vals %>% filter(type == "point"), 
              aes(x = weight, y = value, color = group)) +
    xlab("Weight") + ylab("Cost decrease (%)") +
    scale_color_npg(name = "Decision-making factor") +
    theme_light() +
    theme(panel.grid.major.y = element_blank(),
          panel.border = element_blank(),
...
ggsave("figures/figure2_profit_curve.png",
       bg = "white",
...
fig_list <- lapply(nms, function(fct){
    ...
    ggplot(data = val_to_plot) +
        ...
        xlab("Weight") + ylab("Cost increase (% of best solution)") +
        scale_color_npg(name = "Decision-making factor") +
        theme_light() +
        theme(panel.grid.major.y = element_blank(),
              panel.border = element_blank(),
...
ggsave("figures/S13_interaction.png",
       bg = "white",
...
g2 <- ggplot(vals, aes(x = from, y = to, fill = agreement)) +
    ...
    scale_fill_continuous_sequential(
        limits = c(0, 87), name = expression("IoU (" %*% "100)"), 
        palette = "Plasma") +
    theme_minimal() +
...
fig_list <- lapply(c("Maize", "Paddy", "Sorghum", "Cassava", "Beans"), 
    ...
    ggplot(vals, aes(x = from, y = to, fill = agreement)) +
        ...
        geom_text(aes(from, to, label = round(agreement, 0)), 
                  color = "white", size = 3) +
        scale_fill_continuous_sequential(limits = c(0, 94),
...
ggsave("figures/figure4_prod_growth.png",
       bg = "white",
...
ggsave("figures/figure5_compare.png",
       bg = "white",
       width = 7.4, height = 5.6, dpi = 500)
```

### Figure 6 (Not used anymore)
...
ggplot() + geom_point(data = vals %>% filter(yield == 100),
    ...
    scale_fill_brewer(
        "Increased attainable yield (%)",
        palette="PuBu",
        guide = guide_legend(title.position="top", title.hjust = 0.5)) +
    xlab("Cropland usage rate (%)") + ylab("Multiples of current crop production") +
    theme_classic() +
...
ggsave("figures/figure6_prod_potential.png",
       bg = "white", width = 3, height = 5, dpi = 500)
```
