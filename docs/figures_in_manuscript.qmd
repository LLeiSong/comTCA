...
The figure 1 is drawn in drawio.
...
fig_list <- lapply(c("Y", "I", "B", "Ca", "Co", "D"), function(item){
    ...
    vals <- vals %>% 
        mutate(group = ifelse(
            ...
            ifelse(scenario %in% setdiff(c("B", "Ca", "Co", "D", "Y"), item_mod), 
                   "Single factor", "Hybrid"))) %>% 
        mutate(group = factor(
            group, levels = c("Single factor", "Optimal", "Hybrid"),
            labels = c("Single factor (", "Optimal)", "Hybrid")))
    ...
    ggplot(data = vals) + 
        ...
        scale_color_manual(
            name = "Solution type", 
            values = c(pal_npg("nrc")(3)[2], pal_npg("nrc")(1), 
                       pal_npg("nrc")(3)[3])) +
        theme_light() +
...
ggsave("figures/figure2_factor_compare.png",
       bg = "white",
...
ggsave("figures/figure3_newland_agreement.png",
       bg = "white",
...
smry_sup <- smry_others %>% filter(scenario == "Y") %>% 
    ...
    rbind(smry_others %>% filter(scenario %in% c("B", "Ca", "Co", "D")) %>% 
...
vals <- do.call(rbind, lapply(unique(vals$group)[-5], function(fct){
...
ggsave("figures/figure4_profit_curve.png",
       bg = "white",
...
legend <- ggplot() + 
    ...
    scale_fill_manual('Land usage rate', 
                      ...
                      guide = guide_legend(
                          title.position = "top", 
                          title.hjust = -1, title.vjust = -13, 
                          direction = "horizontal", 
                          order = 1,
                          override.aes = list(size=0))) +
    new_scale_fill() +
    geom_raster(data = dat3, aes(x = x, y = y, fill = z)) +
    scale_fill_manual('',
...
g1 <- ggarrange(legend_lc, ggarrange(plotlist = fig_list, nrow = 1, ncol = 3,
                                     labels = c("A", "B", "C")), 
                nrow = 2, ncol = 1, heights = c(3.2, 10))
