...
vals_cmpr <- read.csv(file.path(f_dir, "summary_scenarios_USG65_mean_sd.csv")) %>% 
    ...
    filter(change == "Y100") %>% 
    mutate(farmable_area_mean = farmable_area_mean / num_unit_mean) %>% 
    select(-c(land_usage, change, num_unit_mean, num_unit_sd))
...
fig_list <- lapply(names(simu_mean)[2:6], function(ele){
    ...
    if (str_detect(ele, "Production")){
        ...
        vals$value <- vals$value / 60 / 1e7
        ylab_nm <- expression(paste("Travel time increase (hours * ", 10^7, ")     "))
    }
...
labels <- sprintf(
    paste0("<span style='font-size:7pt; color:white'>%s.",
           ...
           "color:white'>%s</span>"),
    floor(vals$mean / 1e6), round((vals$mean / 1e6)%%1 * 10, 0))
...
vals_65 <- read.csv(
    ...
    select(-c(scenario, num_unit_mean))
clms <- names(vals_65)
clms <- clms[!str_detect(clms, "_sd")]
...
g2 <- ggplot(data = vals) + 
    geom_linerange(aes(
        ...
        linewidth = 0.8, position = position_dodge2(width = 0.5)) +
    geom_point(aes(x = Practice, y = value, color = group), size = 4,
               position = position_dodge2(width = 0.5)) +
    geom_text(data = vals %>% mutate(value = ifelse(value == 100, NA, value)), 
              aes(x = Practice, y = value, label = round(value, 0)), 
