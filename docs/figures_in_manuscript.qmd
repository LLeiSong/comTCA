...
items_new <- c("Y", "I", "T", "B", "Ca", "Co", "Hybrid")
...
vals_cmpr <- read.csv(file.path(f_dir, "summary_scenarios_USGs.csv")) %>% 
    ...
    mutate(scenario = ifelse(scenario == "D", "T", scenario)) %>% 
    mutate(scenario = factor(
        scenario, levels = items_new, labels = items_new)) %>% 
    mutate(farmable_area_intense = farmable_area / num_unit) %>% 
    select(-c(land_usage, change, num_unit, weight))
names(vals_cmpr) <- c("B", "Ca", "Co", "T", "Y", "scenario", "I")

# Figure
items <- c("Y", "I", "T", "B", "Ca", "Co", "Hybrid")
fig_list <- lapply(c("Y", "I", "T", "B", "Ca", "Co"), function(item){
    # Clean data
    ...
    vals <- vals %>% 
        mutate(group = ifelse(
            ...
            ifelse(scenario %in% setdiff(c("B", "Ca", "Co", "T", "Y"), item_mod), 
    ...
    if (str_detect(item, "Y")){
        ...
        vals$value <- vals$value / 1e6
        xlab_nm <- expression(paste("Connectivity loss (", 10^6, ")"))
    } else if (str_detect(item, "T")){
...
vals_balanced <- vals_cmpr %>% filter(scenario == "Hybrid") %>% 
    select(-c(scenario, I))
...
items_new <- c("Y", "T", "B", "Ca", "Co", "Hybrid")
simus <- do.call(rbind, lapply(items, function(item){
    item_new <- ifelse(item == "D", "T", 
                       ifelse(item == "YBCaCoD", "Hybrid", item))
    read.csv(sprintf("%s/%s_trajectory.csv", f_dir, item)) %>% 
        select(c('production', "Distance", 'Biodiversity', "Carbon", "Connectivity", 
                 "farmable_area")) %>% 
        rbind(data.frame(
            production = 0, Biodiversity = 0, Carbon = 0,
            Connectivity = 0, Distance = 0, farmable_area = 0), .) %>% 
        mutate(mod = item_new)
})) %>% mutate(mod = factor(mod, levels = items_new, labels = items_new))
...
## Figure
items <- c("Y", "T", "B", "Ca", "Co", "Hybrid")
vals <- pairwise_agree %>% filter(crop == "Overall") %>%
    mutate(from = ifelse(
        from == "D", "T", ifelse(from == "YBCaCoD", "Hybrid", from))) %>% 
    mutate(to = ifelse(
        to == "D", "T", ifelse(to == "YBCaCoD", "Hybrid",  to))) %>% 
...
# Get some numbers
vals_to_check <- lapply(unique(vals$group), function(grp){
    vals %>% filter(type == "line") %>% 
        filter(group == grp) %>% 
        arrange(weight) %>% 
        mutate(value_ds = c(0, .$value[1:(nrow(.) - 1)] - .$value[2:nrow(.)]),
               farmable_area_ds = c(0, .$farmable_area[2:nrow(.)] -
                                        .$farmable_area[1:(nrow(.) - 1)]))
}) %>% bind_rows()
...
g1 <- ggplot() +
    ...
    scale_color_manual(name = "Decision-making factor",
...
fig_list <- lapply(nms, function(fct){
    ...
    val_to_plot <- val_to_plot %>% 
        mutate(factor = factor(factor, 
                              levels = c("B", "D", "Ca", "Co"),
                              labels = c("Biodiversity", "Travel time", 
                                         "Carbon", "Connectivity")))
...
ggsave("figures/S16_prod_growth.png",
       bg = "white",
...
legend <- ggplot() + 
    ...
    scale_fill_manual(expression(atop(bold("S"), "Paddy")),,
                      ...
                      guide = guide_legend(
                          theme = theme(
                              legend.text = element_text(color = "#00A087FF", 
                                                         face = "italic"),
...
vals <- vals %>% group_by(Element) %>% 
    ...
    mutate(Practice = factor(Practice, labels = letters[1:3],
...
g2 <- ggplot(data = vals) + 
    ...
    theme(panel.grid.major.x = element_blank(),
          ...
          legend.text = element_text(size = 10, color = "#00A087FF", face = "italic"),
          legend.title = element_text(size = 10, color = "#00A087FF", face = "bold.italic"),
          legend.position = "top",
          legend.margin = margin(rep(0, 4)),
...
ggsave("figures/figure6_compare.png",
       bg = "white",
...
p1 <- ggplot() + geom_spatraster(data = habitat_lv2) +
    theme_void() + theme(legend.position = "none")
p2 <- ggplot() + geom_spatraster(data = habitat_refined) +
    theme_void() + theme(legend.position = "none")
