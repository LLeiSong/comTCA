...
fig_list <- lapply(c("Y", "I", "B", "Ca", "Co", "D"), function(item){
    ...
    vals <- vals %>% 
        ...
        mutate(group = factor(
            group, levels = c("Optimal", "Single factor", "Combined factors"),
            labels = c("Optimal", "Single factor", "Combined factors")))
    ...
    if (str_detect(item, "Y")){
        ...
        xlab_nm <- expression(paste("Travel time increase (hours * ", 10^6, ")            "))}
...
fig_list <- lapply(names(simus)[1:5], function(ele){
    ...
    if (str_detect(ele, "production")){
        ...
        vals$value <- vals$value / 60 / 1e6
        ylab_nm <- expression(paste("Travel time increase (hours * ", 10^6, ")     "))
    }
    ...
    if (str_detect(ele, "production")){
        p <- p + geom_point(aes(x = min(farmable_area), y = pgi_mean), size = 0.8) +
            # geom_errorbar(aes(x = min(farmable_area), y = pgi_mean,
...
g1 <- ggplot(data = vals %>% filter(yield == 100),
    ...
    scale_fill_npg(name = "Land usage (%)") + 
...
legend <-  ggplot() + 
    ...
    scale_fill_manual('Land usage',
                      ...
                      guide = guide_legend(
                          title.position = "top", 
                          title.hjust = 2, direction = "vertical", 
                          order = 1)) +
    new_scale_fill() +
...
vals <- vals %>% group_by(Element) %>% 
    ...
    mutate(Element = factor(
        ...
        labels = c("New cropland area", "Biodiveristy loss", "Carbon loss",
                   "Connectivity loss", "Travel time increase")))
