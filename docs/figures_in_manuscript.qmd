...
# Load dataset
vals_cmpr <- read.csv(file.path(f_dir, "summary_scenarios_USG64_best.csv")) %>% 
    ...
    select(-c(land_usage, change, num_unit, num_unit, iter_n,
              farmable_area_cultv, farmable_area_cultv))
...
fig_list <- lapply(c("Y", "I", "B", "Ca", "Co", "D"), function(item){
    ...
    if (str_detect(item, "Y")){
        ...
        vals$value <- vals$value / 60 / 1e7
        xlab_nm <- expression(paste("Travel time increase (hours * ", 10^7, ")            "))}
...
simu_mean <- do.call(rbind, lapply(items, function(item){
    read.csv(sprintf("%s/%s_Y100_USG64_simu_best.csv", f_dir, item)) %>% 
        mutate(mod = item)
...
fig_list <- lapply(names(simu_mean)[2:6], function(ele){
    ...
    if (str_detect(ele, "Production")){
        vals$value <- vals$value / 1e6 + pgi_mean
        ylab_nm <- expression(paste("Production gain (tons * ", 10^6, ")"))
...
```{r}
# Summarize the maps
fnames <- list.files(f_dir, full.names = TRUE, pattern = "Y100_USG64_best.tif")
...
fig_list <- lapply(c("Maize", "Paddy", "Sorghum", "Cassava", "Beans"), 
    ...
    ggplot(vals, aes(x = from, y = to, fill = agreement)) +
        ...
        geom_text(aes(from, to, label = round(agreement, 0)), 
                  color = "white", size = 3) +
        scale_fill_continuous_sequential(limits = c(0, 87),
            name = expression("IoU (" %*% "100)"), palette = "Plasma") +
        theme_minimal() +
        theme(axis.title.x = element_blank(),
              axis.title.y = element_blank(),
...
legend <-  ggplot() + 
    ...
    scale_fill_manual('Land usage (%)',
                      values = c("grey", "Black"), labels = c("64", "80"),
    )+
...
vals_64 <- read.csv(
    file.path("results/scenarios/summary", 
              "summary_scenarios_USG64_best.csv")) %>% 
    filter(scenario == "YBCaCoD") %>% 
    select(-c(scenario, num_unit, iter_n, farmable_area_cultv))
names(vals_64) <- c("B", "Ca", "Co", "D", "Y", "USG", "Practice")

vals_80 <- read.csv(
    file.path("results/USG80/summary", 
              "summary_scenarios_USG80_best.csv")) %>% 
    filter(scenario == "YBCaCoD") %>% 
    select(-c(scenario, num_unit, iter_n, farmable_area_cultv))
names(vals_80) <- c("B", "Ca", "Co", "D", "Y", "USG", "Practice")

vals <- vals_64 %>% pivot_longer(
...
vals <- vals %>% group_by(Element) %>% 
    ...
    mutate(USG = factor(USG, labels = c("64", "80"), levels = c(64, 80))) %>% 
