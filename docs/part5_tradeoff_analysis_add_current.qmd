...
nms <- c(
    ...
    "elephant_cons_index", "production_gain", 
    "potential_production", "travel_time")
names(inputs_80) <- nms
names(inputs_60) <- nms
...
inputs_80_std <- do.call(c, lapply(names(inputs_80), function(name){
    lyr <- inputs_80[[name]]
    if (name %in% c("potential_production")) {
        lyr <- stretch(lyr, minv = 0, maxv = 1)
    } else if (name == "production_gain") {
        lyr <- lyr
    } else {
...
prod_intense <- rast(file.path(agro_dir, "production_intens.tif"))
...
cbetas_bcep <- c(
    ...
    "potential_production" = 1/8, "travel_time" = 1/8)

# 80 scenario
land_allocate(
    inputs_80_std[["production_gain"]], inputs_80_std, 
    production_need - prod_intense, cbetas_bcep, result_dir,
    "bcep_80")
# 60 scenario
land_allocate(
    inputs_60_std[["production_gain"]], inputs_60_std, 
    production_need - prod_intense, cbetas_bcep, result_dir,
    "bcep_60")
# 60 with no production gain scenario
## failed
land_allocate(
    inputs_60_half_std[["production_gain"]], inputs_60_half_std, 
    production_need - prod_intense / 2, cbetas_bcep, result_dir,
    "bcep_60_half")
...
cbetas_bcp <- c(
    ...
    "potential_production" = 1/6, "travel_time" = 1/6)
...
cbetas_p <- c(
    ...
    "elephant_cons_index" = 0, 
    "potential_production" = 1/2, "travel_time" = 1/2)
