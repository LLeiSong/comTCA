...
nm_tab <- data.frame(
    ...
    show_name = c("Attainable production (T)", "Attainable production gain (T)", 
                  "Biodiversity cost (0-1)", "Carbon cost (T)", 
                  "Elephant migration cost (0-1)", "Travel time (mins)"))
...
names(sum_cost_abc) <- c(
    letters[1:3], "Plant area (ha)", "Biodiveristy cost (0-1)", 
    "Carbon cost (T)", "Elephant migration cost (0-1)")

fg4_hist_list <- lapply(names(sum_cost_abc)[4:7], function(nm){
    tbl <- sum_cost_abc %>% 
...
fg_hist <- ggarrange(
    plotlist = fg4_hist_list, nrow = 1, ncol = 4)
...
crop_change <- c(crop_current, crop_allo_80, crop_allo_60) %>% 
    crop(bry) %>% mask(bry)
names(crop_change) <- c("Current", "Reallocated_80", "Reallocated_60")
...
ggarrange(p80, p60, ncol = 2,
    labels = letters[1:2], font.label = list(size = 12),
...
# Get areas
areas <- rbind(areas_60, areas_80) %>% 
    mutate(scenario = c("60%", "80%"))
...
# Gather all values
vals <- cbind(areas, rbind(costs_60, costs_80)) %>% 
    as_tibble()
