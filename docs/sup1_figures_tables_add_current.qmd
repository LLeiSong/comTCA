...
nm_tab <- data.frame(
    lyr_name = c("potential_production", "travel_time",
                 "biodiversity_cost", "carbon_cost", 
                 "elephant_cons_index"),
    show_name = c("Attainable production (T)", "Travel time (mins)",
...
names(sum_cost_abc) <- c(
    letters[1:3], "No of planning units", "Expand area (ha)", 
...
areas_60 <- values(areas_60) %>% data.frame() %>% 
    ...
    mutate(class = factor(
        ...
        labels = c("Expand current cropland", "Non-cropland to keep",
                   "Current cropland", "New land to expand")))
new_land_to_expand <- (100 * 0.6 - (areas_60 %>% 
...
cropland_to_expand <- sum(cropland_to_expand)
...
areas_60_half <- values(areas_60_half) %>% data.frame() %>% 
    ...
    mutate(class = factor(
        ...
        labels = c("Expand current cropland", "Non-cropland to keep",
                   "Current cropland", "New land to expand")))
new_land_to_expand <- (100 * 0.6 - (areas_60_half %>% 
    filter(class == "New land to expand") %>% 
    pull(area_current))) %>% sum()
...
areas_80 <- values(areas_80) %>% data.frame() %>% 
    ...
    mutate(class = factor(
        ...
        labels = c("Expand current cropland", "Non-cropland to keep",
                   "Current cropland", "New land to expand")))
new_land_to_expand <- (100 * 0.6 - (areas_80 %>% 
    filter(class == "New land to expand") %>% 
    pull(area_current))) %>% sum()
cropland_to_expand <- 100 * 0.6 - (areas_80 %>% 
    filter(class == "Expand current cropland") %>% 
    pull(area_current))
cropland_to_expand[cropland_to_expand < 0] <- 0
cropland_to_expand <- sum(cropland_to_expand)
