...
nm_tab <- data.frame(
    lyr_name = c("potential_production", "attainable_yield", 
                 "biodiversity_cost", "carbon_cost", 
                 "elephant_cons_index", "travel_time"),
    show_name = c("Attainable production (T)", "Attainable yield (t/ha)", 
...
ggsave(file.path("docs/figures_yield", "figure2.png"),
       bg = "white",
...
ggsave(file.path("docs/figures_yield", "figure3.png"),
       bg = "white",
...
ggsave(file.path("docs/figures_yield", "figure4.png"),
       bg = "white",
...
# Get areas
areas <- rbind(areas_60, areas_80, areas_60_half) %>% 
    mutate(scenario = c("60%", "80%", "60%_half"))

## Cost calculation
# 60%
costs <- rast(file.path(result_dir, "tradeoff_inputs_60.tif"))
costs <- subset(costs, 1:4)

fname <- here("results/fut_agro_land_bcep_60.tif")
...
fname <- here("results/fut_agro_land_bcep_60_half.tif")
costs_abcd <- c(rast(fname), costs)
names(costs_abcd)[1] <- "land_alloc"
...
fname <- here("results/fut_agro_land_bcep_80.tif")
costs_abcd <- c(rast(fname), costs)
names(costs_abcd)[1] <- "land_alloc"
...
# Gather all values
vals <- cbind(areas, rbind(costs_60, costs_60_half, costs_80)) %>% 
    as_tibble()
